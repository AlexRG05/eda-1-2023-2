...
void busqueda(struct Node* last, int llave){
    ...
    temp = last->next;
    while(temp->next != last->next && temp->data != llave){
        temp = temp->next;
        
    }
    
    if (temp->data == llave){
      printf("El elemento %d se encuentra en la lista", llave);
      

    }else{
        printf("El elemento %d no se encuentra en la lista", llave);
    }  
...
int main() {
  ...
  last = addFront(last, 14);
  last = addFront(last, 22);
  last = addEnd(last, 7);
  last = addEnd(last, 16);
  last = addAfter(last, 1, 7);
  last = addAfter(last, 45, 14);
  ...
  printf("\nIngrese el valor que quiere buscar en la lista: ");
  scanf("%d", &busc);
  ...
  printf("\nPara volver a usar el programa escriba 0");
  scanf("%d", &cond);
  if (cond != 0){
     break;
  }
